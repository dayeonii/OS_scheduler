...
public class sjf {
    public static ArrayList<SchedulingResult> sjf(ArrayList<process> PCB_list){
        ArrayList<SchedulingResult> sjf_result = new ArrayList<>();
...
//        }
        ...
        while(!sorted_PCB.isEmpty() || !readyQ.isEmpty() || runningProcess!=null) {  //Q를 다 비울때까지 (=모든 프로세스가 실행을 완료)
            ...
            if(!readyQ.isEmpty() && runningProcess == null) {
                //올리기 전에 현재 readyQ에 있는거 burstTime으로 정렬
            ...
            if(runningProcess != null) {
                ...
                if(runningProcess.getBurstTime()==0) {
                    ...
                    sjf_result.add(new SchedulingResult( runningProcess.getPid(), startTime, originBurstTime.get(runningProcess.getPid()), waitTime));
